...
do
    ...
    sudo sed -i "s/upload_max_filesize = .*/upload_max_filesize = 100M/" $INIFILE
    sudo sed -i "s/post_max_size = .*/post_max_size = 100M/" $INIFILE
...
echo "${DBCONF}" > /vagrant/config/dev/db.php
